...
#not sure i can do this because get_sentiments("nrc") returns multiple sentiment values per word, so inner_join duplicates words
...
num_breaks <- 10
temp_sentiments %>%
  count(sentiment, book, sort = TRUE) %>% 
  #mutate(sentiment = fct_reorder(sentiment, n, sum, .desc = TRUE)) %>%
...
temp_sentiments %>%
  ...
  summarise(num_words = n(),
            ...
            average_sentiment_per_word = total_sentiment / num_words) %>% 
  #mutate(sentiment = fct_reorder(sentiment, n, sum, .desc = TRUE)) %>%
  ggplot(aes(x=book, y=average_sentiment_per_word)) +
  geom_col(alpha=0.75) +
  scale_y_continuous(breaks = pretty_breaks(num_breaks), labels = rt_pretty_axes) +
  theme_light(base_size = base_size) +
  theme(axis.text.x=element_text(angle=30, hjust=1),
...
```{r wordcloud, dependson = "tidy_books", fig.cap="Most common positive and negative words in Jane Austen's novels"}
library(reshape2)
...
tidy_books %>%
  ...
  comparison.cloud(colors = rt_colors_good_bad(good_first = FALSE),
                   max.words = 100)
```
...
tidy_books %>%
  ...
  comparison.cloud(colors = rt_colors(),
                   max.words = 100, )
```
```
